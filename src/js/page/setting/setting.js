...
let refreshData = async () => {
  return await axios({
    ...
    res.data = res.data.map((row, i) => {
      let btnDel = `<button type="button" name="del" data-toggle="confirmation" data-original-title="确认删除本数据库?" data-singleton="true" data-btn-ok-label="是" data-btn-cancel-label="否" data-id="${
        row[0]
